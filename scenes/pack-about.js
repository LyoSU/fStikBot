...
packAbout.enter(async (ctx) => {
  await ctx.replyWithHTML(ctx.i18n.t('scenes.packAbout.enter'), {
    reply_markup: Markup.keyboard([
      [
        ctx.i18n.t('scenes.packAbout.pack_info'),
        ctx.i18n.t('scenes.packAbout.user_info')
      ],
      [
...
packAbout.hears(/.*/, async (ctx, next) => {
  if (ctx.message.text === ctx.i18n.t('scenes.packAbout.user_info')) {
    let sharedUserId = ctx?.message?.forward_from?.id || ctx?.message?.users_shared?.user_ids[0]
    ...
    // Use the same user about logic from handlers/user-about.js
  ...
  if (ctx.message.text === ctx.i18n.t('scenes.packAbout.pack_info')) {
    await ctx.replyWithHTML(ctx.i18n.t('scenes.packAbout.send_sticker'), {
      reply_markup: Markup.keyboard([
        [
          ctx.i18n.t('scenes.btn.cancel')
        ]
